...
export default function Register({ onRegisterSuccess, onSwitchToLogin }: RegisterProps) {
    ...
    const validate = () => {
        let ok = true;
        setNameErr(''); setEmailErr(''); setPassErr(''); setConfirmPassErr(''); setGlobalErr('');

        if (!name.trim()) {
            setNameErr('Nama tidak boleh kosong.'); ok = false;
        } else if (name.trim().length < 2) {
            setNameErr('Nama minimal 2 karakter.'); ok = false;
        }
        ...
        if (!passwordConfirmation) {
            setConfirmPassErr('Konfirmasi password tidak boleh kosong.'); ok = false;
        } else if (password !== passwordConfirmation) {
            setConfirmPassErr('Password tidak cocok.'); ok = false;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(err.message ?? 'Pendaftaran gagal. Coba lagi.');
            }

            setGlobalSuccess('Akun berhasil dibuat! Silakan masuk.');
            setTimeout(() => {
                onRegisterSuccess();
            }, 1500);
        } catch (err: any) {
                ...
                <div className="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/60 overflow-hidden">
                    ...
                    <div className="p-7 space-y-6">
                        <div className="text-center">
                            <h1 className="text-xl font-bold text-slate-900 tracking-tight">Buat Akun Baru</h1>
                            <p className="text-sm text-slate-400 mt-1">
                                Daftarkan diri Anda untuk mulai mengelola keuangan
                            </p>
                        </div>
                        ...
                        {globalSuccess && (
                            <div className="flex items-center gap-2.5 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                                ...
                                <p className="text-sm text-emerald-700 font-semibold">{globalSuccess}</p>
                            </div>
                        )}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                            <Field
                                ...
                                type="text"
                                value={name}
                                onChange={(v) => { setName(v); setNameErr(''); setGlobalErr(''); }}
                            ...
                            <Field
                                ...
                                onChange={(v) => { setPasswordConfirmation(v); setConfirmPassErr(''); setGlobalErr(''); }}
                            ...
                            >
                                {isLoading ? (
                                    <><Loader2Icon className="h-4 w-4 animate-spin" />Mendaftarkan...</>
                                ) : (
